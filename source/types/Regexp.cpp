...
namespace slim
{
    ...
    Ptr<Object> MatchData::begin(Number *n)
    {
        ...
        else return make_value(static_cast<long long>(sub.first - str.begin()));
    }
    ...
    Ptr<Object> MatchData::end(Number * n)
    {
        auto sub = get_sub(n);
        if (!sub.matched) return NIL_VALUE;
        else return make_value(static_cast<long long>(sub.second - str.begin())); // FIXME: int?
    }
    Ptr<Object> MatchData::offset(Number * n)
    {
        ...
        return make_array({
                make_value(static_cast<long long>(sub.first - str.begin())),
                make_value(static_cast<long long>(sub.second - str.begin()))
            });
    }
    ...
    {
        return make_value(static_cast<long long>(match.size()));
    }
